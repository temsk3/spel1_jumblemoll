...
export const createCustomer = fucRegion.https.onCall(async (data, context) => {
  const email = data.email;
  const customer = await stripe.customers.create({ email: email }, { idempotencyKey: data.idempotencyKey });
  const customerId = customer.id;
  return { customerId: customerId };
});

// MARK: カード情報の登録
export const createCardInfo = fucRegion.https.onCall((data, context) => {
...
// MARK: - カード情報の取得
export const retrieveCardInfo = fucRegion.https.onCall((data, context) => {
  const customerId = data.customerId;
...
// MARK: カード情報の削除
export const deleteCardInfo = fucRegion.https.onCall((data, context) => {
  const customerId = data.customerId;
  const cardId = data.cardId;
...
// MARK: - 決済を作成する
export const createStripeCharge = fucRegion.https.onCall(async (data, context) => {
...
export const updateConnectAccount = fucRegion.https.onCall(async (data, context) => {
...
export const retrieveConnectAccount = fucRegion.https.onCall((data, context) => {
  return stripe.accounts.retrieve(data.accountId, { idempotencyKey: data.idempotencyKey });
});
...
export const updateStripeAccountStatus = fucRegion.https.onRequest(async (req, res) => {
